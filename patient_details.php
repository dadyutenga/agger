...
<body class="bg-light">
    ...
    <script>
        ...
        function updateVitalSigns() {
            fetch('vital_signs_api.php?patient_id=<?php echo $patient_id; ?>')
                .then(response => response.json())
                .then(data => {
                    // Update heart rate
                    document.getElementById('heart-rate').textContent = data.heart_rate + ' BPM';
                    updateVitalSignStatus('heart-rate', data.heart_rate, 60, 100);
                    
                    // Update SpO2
                    document.getElementById('spo2').textContent = data.spo2 + '%';
                    updateVitalSignStatus('spo2', data.spo2, 95, 100);
                    
                    // Update temperature
                    document.getElementById('temperature').textContent = data.temperature + '°C';
                    updateVitalSignStatus('temperature', data.temperature, 36.5, 37.5);
        ...
        // Function to update vital sign status and color
        function updateVitalSignStatus(elementId, value, min, max) {
            const element = document.getElementById(elementId);
            const container = element.closest('.vital-sign');
        ...
        // Function to check for critical values and show alerts
        function checkCriticalValues(data) {
            const alerts = [];
            
            if (data.heart_rate < 60 || data.heart_rate > 100) {
                alerts.push(`Heart rate is ${data.heart_rate} (Normal range: 60-100)`);
            }
            
            if (data.spo2 < 95) {
                alerts.push(`SpO2 is ${data.spo2}% (Normal range: 95-100%)`);
            }
            
            if (data.temperature < 36.5 || data.temperature > 37.5) {
                alerts.push(`Temperature is ${data.temperature}°C (Normal range: 36.5-37.5°C)`);
            }
